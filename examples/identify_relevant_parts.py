...
from shape_warping.skill_transfer import infer_relpose, DemoScene, TestScene, get_interaction_points, warp_interaction_points
from shape_warping.shape_reconstruction import ObjectWarpingSE3Batch, ObjectSE3Batch, warp_to_pcd_se3, ObjectWarpingSE2Batch, warp_to_pcd_se3_hemisphere, warp_to_pcd_se2, mask_and_cost_batch_pt 
...
def reconstruct_part_shape(warp_model, 
                target_pcd, 
                target_labels, 
                canon_labels, 
                inference_kwargs, 
    ...
    if viz:
        for i, label in enumerate(target_labels):
            fig = viz_utils.show_pcds_plotly({f"label_{i}_target_0": target_pcd[label==0], 
                                        f"label_{i}_target_1": target_pcd[label==1],
                                        f"label_{i}_reconstruction_0": reconstructed_pcd[canon_labels[i]==0],
                                        f"label_{i}_reconstruction_1": reconstructed_pcd[canon_labels[i]==1],
                                        })
            fig.show()
...
def get_relational_labels(part_pcds, warp_models, part_relationships, include_z = False):
    ...
    child_relational_labels = utils.get_part_labels(
                [{p: part_pcds['child'][p] for p in relation} for relation in part_relationships['child']],
                include_z = include_z
            )
    
    parent_relational_labels = utils.get_part_labels(
                [{p: part_pcds['parent'][p] for p in relation} for relation in part_relationships['parent']],
                include_z = include_z
            )
    
    canon_child_relational_labels = utils.get_canon_labels(
                [{p: warp_models['child'][p].canonical_pcl for p in relation} for relation in part_relationships['child']], 
                warp_models['child'],
    ...
    canon_parent_relational_labels = utils.get_canon_labels(
                [{p: warp_models['parent'][p].canonical_pcl for p in relation} for relation in part_relationships['parent']], 
                warp_models['parent'],
    ...
    return child_relational_labels, parent_relational_labels, canon_child_relational_labels, canon_parent_relational_labels

def construct_heuristic(part_pairs, 
                        demo,
                        warp_models, 
                        part_pcds, 
                        part_relationships, 
    ...
    child_relational_labels, parent_relational_labels, canon_child_relational_labels, canon_parent_relational_labels = get_relational_labels(part_pcds, warp_models, part_relationships)
    ...
    for pair in part_pairs:
        ...
        child_reconstruction = demo.warp_models["child"][child_part].to_transformed_pcd(demo.child_reconstruction_params[child_part])
        parent_reconstruction = demo.warp_models["parent"][parent_part].to_transformed_pcd(demo.parent_reconstruction_params[parent_part])
    ...
    constraint_shape = utils.ConstraintShape.from_part_reconstructions(constraint_component_pcds, transform_estimates)

    if viz:
        viz_parent = {part: part_pcds['parent'][part] for part in part_pcds['parent']}
        fig = viz_utils.show_pcds_plotly({'constraint': constraint_shape.canonical_pcl,} | viz_parent)
        fig.show()

    
    source_pcds = [part_pcds['child'][part[0]] for part in [pair for pair in part_pairs]]
    target_pcds = [constraint_component_pcds[part] for part in [pair for pair in part_pairs]]
    ...
    cost_function = (
                ...
                mask_and_cost_batch_pt(
                    ...
                    target,
                    [source_labels], 
                    switch_sides=True)) #To handle the fact we're inverting the transform

    return constraint_shape, cost_function

def test_heuristic(warp_models, demo, test_scene, relevant_parts, viz=False):

    constraint_shape, cost_function = construct_heuristic(relevant_parts, 
                                        demo,
                                        warp_models, 
    ...
    alignment_obs = np.concatenate([test_scene.segmented_pcds['child'][part[0]] for part in relevant_parts])
    ...
    final_transform = constraint_shape.center_transform @ np.linalg.inv(utils.pos_quat_to_transform(combined_params.position, 
                                                                combined_params.quat))

    #record the similarity of the reconstructed pose 
    if viz:
        fig = viz_utils.show_pcds_plotly({'parent': demo.final_pcds['parent'], 
                                        'aligned_alignment_observation': utils.transform_pcd(alignment_obs, final_transform, True),
                                        'constraint': utils.transform_pcd(constraint_shape.canonical_pcl, constraint_shape.center_transform, True),
                                        'child': utils.transform_pcd(demo.initial_pcds['child'], final_transform, True)})
        fig.show()
...
def get_demo_interaction_points_by_parts(demo, warp_models, viz=False):
    ...
    child_relational_labels, parent_relational_labels, canon_child_relational_labels, canon_parent_relational_labels = get_relational_labels(demo.segmented_initial_pcds, warp_models, demo.part_relationships, include_z = True)

    if viz:
        fig = viz_utils.show_pcds_plotly({'child_cup_0': demo.segmented_final_pcds['child']['cup'][child_relational_labels['cup'][0]==0], 
                                        'child_cup_1': demo.segmented_final_pcds['child']['cup'][child_relational_labels['cup'][0]==1],
                                        'child_handle_0': demo.segmented_final_pcds['child']['handle'][child_relational_labels['handle'][0]==0],
                                        'child_handle_1': demo.segmented_final_pcds['child']['handle'][child_relational_labels['handle'][0]==1],
                                        'parent_branch_0': demo.segmented_final_pcds['parent']['branch'][parent_relational_labels['branch'][0]==0],
                                        'parent_branch_1': demo.segmented_final_pcds['parent']['branch'][parent_relational_labels['branch'][0]==1],
                                        'parent_trunk_0': demo.segmented_final_pcds['parent']['trunk'][parent_relational_labels['trunk'][0]==0],
                                        'parent_trunk_1': demo.segmented_final_pcds['parent']['trunk'][parent_relational_labels['trunk'][0]==1],

                                        })
        fig.show()

        fig = viz_utils.show_pcds_plotly({'canon_cup_0': warp_models['child']['cup'].canonical_pcl[canon_child_relational_labels['cup'][0]==0],
                                        'canon_cup_1': warp_models['child']['cup'].canonical_pcl[canon_child_relational_labels['cup'][0]==1],
                                        'canon_handle_0': warp_models['child']['handle'].canonical_pcl[canon_child_relational_labels['handle'][0]==0],
                                        'canon_handle_1': warp_models['child']['handle'].canonical_pcl[canon_child_relational_labels['handle'][0]==1],
                                        'canon_branch_0': warp_models['parent']['branch'].canonical_pcl[canon_parent_relational_labels['branch'][0]==0],
                                        'canon_branch_1': warp_models['parent']['branch'].canonical_pcl[canon_parent_relational_labels['branch'][0]==1],
                                        'canon_trunk_0': warp_models['parent']['trunk'].canonical_pcl[canon_parent_relational_labels['trunk'][0]==0],
                                        'canon_trunk_1': warp_models['parent']['trunk'].canonical_pcl[canon_parent_relational_labels['trunk'][0]==1],
    ...
    for part in demo.parent_part_names:
        part_reconstruction, part_params = reconstruct_part_shape(warp_models['parent'][part],
    ...
    for part in demo.child_part_names:
        part_reconstruction, part_params = reconstruct_part_shape(warp_models['child'][part],
        ...
        part_params = utils.update_reconstruction_params_with_transform(part_params, demo.child_transform)
    ...
    for part_pair in all_part_pairs:
        ...
        part_pair_interaction_points[parent_part][child_part] = {'knns': knns, 'deltas': deltas, 'target_indices': i_2}
        
        warped_part_pair_interaction_points = warp_interaction_points(part_pair_interaction_points[parent_part][child_part], 
                                                    warp_models['child'][child_part], 
                                                    warp_models['parent'][parent_part], 
                                                    demo_reconstruction_params['child'][child_part], 
                                                    demo_reconstruction_params['parent'][parent_part])
        
        fig = viz_utils.show_pcds_plotly({'child': demo.segmented_final_pcds['child'][child_part], 
                                          ...
                                          'child_interaction_points': warped_part_pair_interaction_points['child'],
                                          'parent_interaction_points': warped_part_pair_interaction_points['parent'],
                                          })
        fig.show()
...
if __name__ == "__main__":
    demo_path = osp.join('example_data', 'mug_on_rack_demos', 'mug_on_tree_demonstration.pkl')
    ...
    warp_models = {
        'parent': {
            'trunk': utils.CanonShape.from_pickle("example_data/rack_models/example_pretrained_trunk"),
            'branch': utils.CanonShape.from_pickle("example_data/rack_models/example_pretrained_branch"),
        },

        'child': {
            'cup': utils.CanonShape.from_pickle("example_data/mug_models/example_pretrained_cup"),
            'handle': utils.CanonShape.from_pickle("example_data/mug_models/example_pretrained_handle"),
        }
    }

    if demo.interaction_points is None:
        demo.interaction_points = get_demo_interaction_points_by_parts(demo, warp_models)
    ...
    cost = test_heuristic(warp_models, demo, TestScene(demo.initial_pcds, demo.segmented_initial_pcds), test_part_pair)
    print(cost)
