...
part_warp_models = {'body': pickle.load(open("example_data/teapot_models/example_pretrained_body", "rb")),
			   'handle': pickle.load(open("example_data/teapot_models/example_pretrained_handle", "rb")),
			   'lid': pickle.load(open("example_data/teapot_models/example_pretrained_lid", "rb")),
			   'spout': pickle.load(open("example_data/teapot_models/example_pretrained_spout", "rb"))}

test_object_parts = {'body': trimesh.load("example_data/teapot_meshes/kettle_5/kettle_5_body.obj"),
...
test_object_pcls = {part_name: utils.trimesh_create_verts_surface(test_object_parts[part_name], 1000)\
             for part_name in test_object_parts.keys()}

# Scaling to match
test_object_pcls['body'], test_object_pcls['spout'], test_object_pcls['handle'], test_object_pcls['lid'] =\
    utils.scale_points_circle([test_object_pcls['body'], test_object_pcls['spout'], test_object_pcls['handle'], test_object_pcls['lid']], base_scale=.2)


viz_utils.show_pcds_plotly(
    {
        f"Source (Canonical) {part} Pointcloud": \
            utils.transform_pcd(part_warp_models[part].canonical_pcl,
                                utils.pos_quat_to_transform(part_warp_models[part].center_transform, (0,0,0,1))) \
...
for part_name in object_parts:
    ...
    part_reconstructed[part_name], _, part_params[part_name] = warp_to_pcd_se3(
        part_warps[part_name], n_angles=5, n_batches=12, inference_kwargs=inference_kwargs
    )

    viz_pointclouds = viz_pointclouds | {
        f"Source (Canonical) {part_name} Pointcloud": part_warp_models[part_name].canonical_pcl,
    ...
    viz_mesh_vertices = viz_mesh_vertices | {
        ...
        f"{part_name} Reconstruction": part_warp_models[part_name].to_transformed_mesh(part_params[part_name]).vertices
    }
    viz_mesh_faces = viz_mesh_faces | {
        ...
        f"{part_name} Reconstruction": part_warp_models[part_name].to_transformed_mesh(part_params[part_name]).faces,
    }
...
# See the meshes
viz_utils.show_meshes_plotly(
    viz_mesh_vertices, viz_mesh_faces
).show()
