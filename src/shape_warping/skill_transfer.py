...
class TestScene:
    ...
    def __init__(self, pcds, segmented_pcds):
        ...
        for object_type in ("parent", "child"):
            self.pcds[object_type], _ = utils.farthest_point_sample(
                self.pcds[object_type],
                min(DemoScene.max_points_per_object, len(self.pcds[object_type])),
            )
        for object_type in ("parent", "child"):
            for part in self.segmented_pcds[object_type].keys():
                self.segmented_pcds[object_type][part], _ = utils.farthest_point_sample(
                    self.segmented_pcds[object_type][part],
                    min(
                        DemoScene.max_points_per_object,
                        len(self.segmented_pcds[object_type][part]),
                    ),
                )
    ...
    def get_part_relationships(self, num_close_threshold=20, distance_threshold=0.05):
        ...
        all_child_part_pairs = itertools.combinations(
            self.segmented_pcds["child"].keys(), r=2
        )
        all_parent_part_pairs = itertools.combinations(
            self.segmented_pcds["parent"].keys(), r=2
        )

        self.child_part_relationships = []
        for pair in all_child_part_pairs:
            # Get the distance between closest points on the two objects
            dist = utils.nearest_neighbor_distances(
                self.segmented_pcds["child"][pair[0]],
                self.segmented_pcds["child"][pair[1]],
            )
        ...
        for pair in all_parent_part_pairs:
            # Get the distance between closest points on the two objects
            dist = utils.nearest_neighbor_distances(
                self.segmented_pcds["parent"][pair[0]],
                self.segmented_pcds["parent"][pair[1]],
            )
            close_points = dist < distance_threshold
            if np.sum(close_points) > num_close_threshold:
    ...
    def from_pickle(path):
        ...
        segmented_pcds = {
            "parent": {
                part: test_scene["segmented_pcd"].item()["parent"][part]
                for part in test_scene["segmented_pcd"].item()["parent"]
            },
            "child": {
                part: test_scene["segmented_pcd"].item()["child"][part]
                for part in test_scene["segmented_pcd"].item()["child"]
            },
        }
...
class DemoScene:
    ...
    def __init__(
        ...
        self.parent_part_names = sorted(
            list(self.segmented_final_pcds["parent"].keys())
        )

        self.parent_transform = np.matmul(
            utils.pos_quat_to_transform(
                self.final_poses["parent"][:3], self.final_poses["parent"][3:]
            ),
            np.linalg.inv(
                utils.pos_quat_to_transform(
                    self.start_poses["parent"][:3], self.start_poses["parent"][3:]
                )
            ),
        )
        self.child_transform = np.matmul(
            utils.pos_quat_to_transform(
                self.final_poses["child"][:3], self.final_poses["child"][3:]
            ),
            np.linalg.inv(
                utils.pos_quat_to_transform(
                    self.start_poses["child"][:3], self.start_poses["child"][3:]
                )
            ),
        )
    ...
    def get_part_relationships(self, num_close_threshold=20, distance_threshold=0.05):
        # Generate all possible pairs of parts
        all_child_part_pairs = itertools.combinations(
            self.segmented_initial_pcds["child"].keys(), r=2
        )
        all_parent_part_pairs = itertools.combinations(
            self.segmented_initial_pcds["parent"].keys(), r=2
        )
        ...
        for pair in all_child_part_pairs:
            # Get the distance between closest points on the two objects
            dist = utils.nearest_neighbor_distances(
                self.segmented_initial_pcds["child"][pair[0]],
                self.segmented_initial_pcds["child"][pair[1]],
            )
            close_points = dist < distance_threshold
            if np.sum(close_points) > num_close_threshold:
                self.child_part_relationships.append(pair)
        ...
        for pair in all_parent_part_pairs:
            # Get the distance between closest points on the two objects
            dist = utils.nearest_neighbor_distances(
                self.segmented_initial_pcds["parent"][pair[0]],
                self.segmented_initial_pcds["parent"][pair[1]],
            )
            close_points = dist < distance_threshold
            if np.sum(close_points) > num_close_threshold:
                self.parent_part_relationships.append(pair)
    ...
    def from_pickle(path):
        ...
        segmented_initial_pcds = {
            "parent": {
                part: demo["segmented_start_pcds"]["parent"][part]
                for part in demo["segmented_start_pcds"]["parent"]
            },
            "child": {
                part: demo["segmented_start_pcds"]["child"][part]
                for part in demo["segmented_start_pcds"]["child"]
            },
        }

        segmented_final_pcds = {
            "parent": {
                part: demo["segmented_final_pcds"]["parent"][part]
                for part in demo["segmented_final_pcds"]["parent"]
            },
            "child": {
                part: demo["segmented_final_pcds"]["child"][part]
                for part in demo["segmented_final_pcds"]["child"]
            },
        }
        ...
        for object_type in ("parent", "child"):
            for part in segmented_initial_pcds[object_type].keys():
                segmented_initial_pcds[object_type][part], _ = (
                    utils.farthest_point_sample(
                        segmented_initial_pcds[object_type][part],
                        min(
                            DemoScene.max_points_per_object,
                            len(segmented_initial_pcds[object_type][part]),
                        ),
                    )
                )
                segmented_final_pcds[object_type][part], _ = (
                    utils.farthest_point_sample(
                        segmented_final_pcds[object_type][part],
                        min(
                            DemoScene.max_points_per_object,
                            len(segmented_final_pcds[object_type][part]),
                        ),
                    )
                )
...
def get_knn_and_deltas(obj, vps, k=10, show=False):
    ...
    if show:
        viz_utils.show_pcds_pyplot({"obj": obj, "vps": vps})
...
def get_interaction_points(
    ...
    pos_target_target_coords = utils.transform_pcd(
        pos_target, np.linalg.inv(target_obj_param.get_transform())
    )
    # Points on target in canonical source object coordinates.
    pos_target_source_coords = utils.transform_pcd(
        pos_target, np.linalg.inv(source_obj_param.get_transform())
    )
    ...
    dist_2 = np.sqrt(
        np.sum(
            np.square(full_target_pcd[:, None] - pos_target_target_coords[None]), axis=2
        )
    )
...
def mesh_edit_interaction_points(
    interaction_points, edited_child_mesh, edited_parent_mesh, child_pose, parent_pose
):
    ...
    anchors = edited_child_mesh.vertices[knns]
    targets_child = np.mean(anchors + deltas, axis=1)
    targets_parent = edited_parent_mesh.vertices[target_indices]

    child_transform = child_pose
    parent_transform = parent_pose

    child_targets = utils.transform_pcd(targets_child, child_transform)

    parent_targets = utils.transform_pcd(targets_parent, parent_transform)

    return {"parent": parent_targets, "child": child_targets}


def warp_interaction_points(
    interaction_points,
    child_model,
    parent_model,
    ...
    # Warping the interaction points to the object shape
    anchors = child_model.to_pcd(child_reconstruction_params)[knns]
    targets_parent = np.mean(anchors + deltas, axis=1)
    targets_child = parent_model.to_pcd(parent_reconstruction_params)[target_indices]

    child_transform = utils.pos_quat_to_transform(
        child_reconstruction_params.position, child_reconstruction_params.quat
    )
    parent_transform = utils.pos_quat_to_transform(
        parent_reconstruction_params.position, parent_reconstruction_params.quat
    )

    child_targets = utils.transform_pcd(targets_child, parent_transform)

    parent_targets = utils.transform_pcd(targets_parent, child_transform)

    return {"parent": parent_targets, "child": child_targets}


# TODO: I think there may be a mismatch in the child and parent here from the original interaction warping code
def mesh_edit_infer_relpose(
    interaction_points,
    edited_child_mesh,
    edited_parent_mesh,
    child_mesh_pose,
...
def infer_relpose(
    ...
    anchors = child_model.to_pcd(child_reconstruction_params)[knns]
    targets_child = np.mean(anchors + deltas, axis=1)
    targets_parent = parent_model.to_pcd(parent_reconstruction_params)[target_indices]
...
def mesh_transform_interaction_points(
    interaction_points, child_mesh_vertices, parent_mesh_vertices
):
...
def load_all_shapenet_files(obj_type):
    ...
    mesh_data_dirs = {k: osp.join("example_data", v) for k, v in mesh_data_dirs.items()}
    # bad_ids = {
    #     "syn_rack_easy": [],
